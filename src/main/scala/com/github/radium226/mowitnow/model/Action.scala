  ...
  case class MoveForward() extends Action {
    ...
    def apply(state: State)(size: Size): Try[State] = {
      ...
      val position = orientation match {
        case NorthOrientation() => Position(x, min(y + 1, height))
        case EastOrientation() => Position(min(x + 1, width), y)
        case WestOrientation() => Position(max(x - 1, 0), y)
        case SouthOrientation() => Position(x, max(y - 1, 0))
        case _ => Position(x, y)
      }
